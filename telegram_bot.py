...
# متغیرهای سراسری برای توکن و ادمین‌ها — قابل تغییر از طریق پنل وب
BOT_TOKEN = ""
ADMIN_IDS: set = set()
...
def set_bot_config(token: str, admin_ids_str: str):
    """تنظیم توکن و ID ادمین‌ها از داخل کد (بدون نیاز به متغیر محیطی)."""
    global BOT_TOKEN, ADMIN_IDS, API_BASE
    BOT_TOKEN = (token or "").strip()
    _admin_ids_raw = (admin_ids_str or "").strip()
    ADMIN_IDS = {int(x) for x in _admin_ids_raw.replace(" ", "").split(",") if x.isdigit()} if _admin_ids_raw else set()
    API_BASE = f"https://api.telegram.org/bot{BOT_TOKEN}" if BOT_TOKEN else ""
    logger.info(f"Telegram bot config updated: token={'***' if BOT_TOKEN else 'None'}, admins={len(ADMIN_IDS)}")
...
# ── Check config by UUID (for non-admin users) ────────────────────────────────
UUID_PATTERN = re.compile(r"^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$", re.IGNORECASE)

def _is_uuid(text: str) -> bool:
    """بررسی می‌کنه که آیا متن ورودی یه UUID معتبر هست یا نه."""
    return bool(UUID_PATTERN.match(text.strip()))

def _format_user_status(uid: str, l: dict) -> str:
    """فرمت نمایش وضعیت کانفیگ برای کاربر عادی."""
    ...
    remaining = "نامحدود" if not l.get("limit_bytes") else fmt_bytes(max(0, l.get("limit_bytes", 0) - l.get("used_bytes", 0)))
    ...
    if exp:
        try:
            ...
            if delta.total_seconds() > 0:
                ...
                exp_txt = f"{exp_date.strftime('%Y-%m-%d')} ({days_left} روز باقی‌مانده)"
            else:
                exp_txt = f"{exp_date.strftime('%Y-%m-%d')} (منقضی شده)"
        except Exception:
...
async def _handle_message(msg: dict):
    ...
    # کاربران عادی می‌تونن UUID کانفیگ خودشون رو بفرستن و وضعیت رو ببینن
    if not _is_admin(chat_id):
        # اگر UUID معتبر فرستاده شده، وضعیت کانفیگ رو نشون بده
        if _is_uuid(text):
            uid = text.strip()
            l = LINKS.get(uid)
            if not l:
                await _send(chat_id, "❌ کانفیگی با این UUID پیدا نشد.\nلطفاً UUID صحیح رو وارد کنید.")
                return
            await _send(chat_id, _format_user_status(uid, l))
            return
        else:
            await _send(chat_id, "⛔ شما اجازه‌ی دسترسی به این ربات رو ندارید.\n\n💡 برای مشاهده وضعیت کانفیگ خود، UUID اون رو ارسال کنید.")
            return

    if text in ("/start", "/menu"):
        _pending.pop(chat_id, None)
        await _send(chat_id, "👋 به ربات مدیریت A.A-C-P خوش اومدی.\nاز دکمه‌های زیر برای مدیریت کانفیگ‌ها استفاده کن:", _main_menu_kb())
        return
...
async def _handle_callback(cb: dict):
    ...
    if data == "menu":
        _pending.pop(chat_id, None)
        await _edit(chat_id, message_id, "منوی مدیریت A.A-C-P:", _main_menu_kb())
        return
